...
public class SplitHouses {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < input.length(); i++) {
            if (input.charAt(i - 1) == 'H' && input.charAt(i) == 'H') {
                System.out.println("NO");
                return;
            }
        }
        System.out.println("YES");
        System.out.println(input.replace(".", "B"));
    }
}
...
Now, you are required to put some fences that can be marked as
 on some blank spaces so that the village can be divided into several pieces. A person cannot
 walk past a fence but can walk through a house.
...
The number of grids each person can reach is the same and it includes the grid in which the house
is situated.
In order to show that you are enthusiastic and if there are many answers, then you are required to
print the one where most fences are placed.
